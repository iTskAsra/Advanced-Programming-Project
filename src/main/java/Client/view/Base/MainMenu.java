...
public class MainMenu extends view.Base.Menu {
    public MainMenu() {
        ...
        if (view.Base.Main.checkLoggedIn() != null){
            ...
            else if (view.Base.Main.checkLoggedIn().equals("Seller")){
                submenus.put(1, new SellerPanel(this));
            }
            else if (view.Base.Main.checkLoggedIn().equals("Admin")){
    ...
    protected view.Base.Menu logout() {
        return new view.Base.Menu("Logout",this) {
            @Override
            public void show() {
