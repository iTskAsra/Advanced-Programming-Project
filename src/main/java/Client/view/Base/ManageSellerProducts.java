...
import view.Base.ProductPage;
import view.Base.SortHandler;

import java.util.ArrayList;
import java.util.HashMap;

public class ManageSellerProducts extends view.Base.Menu {
    public ManageSellerProducts(String name, view.Base.Menu parentMenu) {
    ...
    private view.Base.Menu removeProduct() {
        return new view.Base.Menu("Remove Product", this) {
            @Override
            public void show() {
    ...
    private view.Base.Menu addProductRequest() {
        return new view.Base.Menu("Add Product Request", this) {
            @Override
            public void show() {
                System.out.println(this.getName() + ":");
            }
            ...
            public void run() {
                ...
                String name = view.Base.Main.scanInput("String");
                System.out.println("Enter product company:");
                String company = view.Base.Main.scanInput("String");
                System.out.println("Enter product price:");
                double price = Double.parseDouble(view.Base.Main.scanInput("double"));
                System.out.println("Enter product quantity:");
                int quantity = Integer.parseInt(view.Base.Main.scanInput("int"));
                System.out.println("Enter product category:");
                String category = view.Base.Main.scanInput("String");
                while (!SellerController.checkIfCategoryExists(category)) {
                    System.out.println("No category with this name, try again:");
                    category = view.Base.Main.scanInput("String");
                }
                ...
                try {
                    ...
                    for (Feature i : category1.getFeatures()) {
                        ...
                        String value = view.Base.Main.scanInput("String");
                        Feature feature = new Feature(i.getParameter(), value);
                        productFeatures.add(feature);
                    }
                    System.out.println("Enter product description:");
                    String description = view.Base.Main.scanInput("String");
                    Product product = new Product(ProductOrOffCondition.PENDING_TO_CREATE, name, company, price, null, quantity, category1, productFeatures, description, new ArrayList<Rate>(), new ArrayList<Comment>(), view.Base.Main.localDateTime.format(view.Base.Main.dateTimeFormatter), price);
    ...
    private view.Base.Menu editProductRequest() {
        return new view.Base.Menu("Edit Product Info Request", this) {
            @Override
            public void show() {
                System.out.println(this.getName() + ":");
                System.out.println("Enter product ID :");
    ...
    private view.Base.Menu showProductBuyers() {
        return new view.Base.Menu("Show Product Buyers", this) {
            @Override
            public void show() {
                System.out.println(this.getName() + ":");
                System.out.println("Enter product ID :");
            ...
            public void run() {
                ...
                try {
                    ...
                    while (view.Base.Main.scanInput("String").trim().equalsIgnoreCase("yes")) {
                        view.Base.SortHandler.sortSellLogs();
    ...
    private view.Base.Menu showProductDetails() {
        return new view.Base.Menu("Show Product Details", this) {
            @Override
            public void show() {
                System.out.println(this.getName() + ":");
                System.out.println("Enter product ID :");
            ...
            public void run() {
                ...
                try {
                    ...
                    view.Base.ProductPage productPage = new ProductPage(product.getName(), this);
                    productPage.show();
                    productPage.run();
                } catch (ExceptionsLibrary.NoProductException e) {
                    System.out.println(e.getMessage());
    ...
    private view.Base.Menu showSellerProducts() {
