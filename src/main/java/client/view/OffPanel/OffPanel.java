...
import client.view.Base.Main;
import client.view.HelpWindow.Help;
import client.view.MainMenuStage.CheckFields;
...
public class OffPanel implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        sort.getSelectionModel().selectedItemProperty().addListener((v, oldValue, newValue) -> {
            OffPageController.getCurrentSort().clear();
            OffPageController.setSort((String) sort.getValue());
            updateProducts();
        });
            ...
            listView.setOnMouseClicked(e -> {
                if (e.getClickCount() == 2) {
                    if (listView.getSelectionModel().getSelectedItem() != null) {
                        OffPageController.disableFilter(listView.getSelectionModel().getSelectedIndex() + 1);
    ...
    private void updateFilterList() {
        ...
        ObservableList<String> filters = FXCollections.observableArrayList(OffPageController.getCurrentFilters());
    ...
    private void updateProducts() {
        ...
        try {
            products = OffPageController.showProducts();
            for (Product i : products) {
                ...
                Off off = OffPageController.offDetails(i.getProductId());
                Label startDateLabel = new Label(off.getStartDate());
                Label endDateLabel = new Label(off.getEndDate());
                Label offAmount = new Label("With Off: " + String.valueOf(i.getPriceWithOff()));
                offAmount.setStyle("-fx-text-fill: firebrick");
                Label remainingTime = new Label();
                DateTimeFormatter dateTimeFormatter =  DateTimeFormatter.ofPattern("yyyy-MM-dd");
                LocalDate startDate = LocalDate.parse(off.getStartDate().substring(0,10),dateTimeFormatter);
                LocalDate endDate = LocalDate.parse(off.getEndDate().substring(0,10),dateTimeFormatter);
                LocalDate localDate = LocalDate.now();
                long diff = ChronoUnit.DAYS.between(localDate,endDate);
                remainingTime.setText(String.valueOf(diff) + " Days");
                vBox.getChildren().addAll(imageView, name, price, rate, offAmount, startDateLabel,endDateLabel, remainingTime);
                ...
                vBox.setOnMouseClicked(e -> {
                    ...
                    try {
                        product = OffPageController.goToProductPage(Integer.parseInt(vBox.getId()));
                ...
                pane.setOnMouseMoved( e -> {
                    try {
                        ...
                        updateFilterList();
                    } catch (ExceptionsLibrary.NoOffException | ParseException | ExceptionsLibrary.NoProductException | ExceptionsLibrary.NoAccountException noOffException) {
                        noOffException.printStackTrace();
                    }
                });

            }
        } catch (ExceptionsLibrary.NoProductException | ExceptionsLibrary.NoFilterWithThisName | ExceptionsLibrary.NoAccountException e) {
            ErrorBoxStart.errorRun(e);
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public void searchApplyClicked() {
        Iterator iterator = OffPageController.getCurrentFilters().iterator();
        while (iterator.hasNext()) {
            String temp = (String) iterator.next();
        ...
        OffPageController.getCurrentFilters().add("Search--" + search.getText());
        updateFilterList();
    }
    ...
    public void availabilityApplyClicked() {
        ...
        if (isSelected) {
            ...
            OffPageController.getCurrentFilters().add("Availability--yes");
        } else {
            if (OffPageController.getCurrentFilters().contains("Availability--yes")) {
                OffPageController.getCurrentFilters().remove("Availability--yes");
            }
        }
        updateFilterList();
    ...
    public void brandApplyClicked() {
        ...
        OffPageController.getCurrentFilters().add("Brand--" + brandName);
        updateFilterList();
    }
    ...
    public void sellerApplyClicked() {
        ...
        try {
            if (GetDataFromDatabase.getAccount(sellerName) != null) {
                OffPageController.getCurrentFilters().add("Seller--" + sellerName);
                updateFilterList();
            } else {
    ...
    public void featureApplyClicked() {
        ...
        OffPageController.getCurrentFilters().add("Feature--" + category + "--" + feature + "--" + featureValueString);
        updateFilterList();
    }

    public void categoryApplyClicked(TreeItem treeItem) {
        OffPageController.getCurrentFilters().add("Category--" + treeItem.getValue());
        updateFilterList();
    }
