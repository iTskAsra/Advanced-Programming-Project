...
import Server.ServerController.GetDataFromDatabaseServerSide;
...
public class SetDataToDatabase {
    ...
    public static void updateSellerOfProduct(Product product, int code) throws ExceptionsLibrary.NoAccountException, ExceptionsLibrary.NoProductException {
        ...
        if (code == 0) {
            for (File i : sellers.listFiles(fileFilter)) {
                ...
                Seller seller = null;
                try {
                    seller = (Seller) GetDataFromDatabaseServerSide.getAccount(sellerName);
                } catch (Server.ServerController.ExceptionsLibrary.NoAccountException e) {
                    e.printStackTrace();
                }
            ...
            for (File i : sellers.listFiles(fileFilter)) {
                ...
                for (Off j : seller.getSellerOffs()) {
                    ...
                    while (iterator.hasNext()) {
                        ...
                        try {
                            tempProduct = GetDataFromDatabaseServerSide.getProduct(Integer.parseInt(tempProductID));
                        } catch (Server.ServerController.ExceptionsLibrary.NoProductException e) {
                            e.printStackTrace();
                        }
                        if (tempProduct.getProductId() == product.getProductId()) {
                            iterator.remove();
    ...
    public static void removeOff(int offId) throws ExceptionsLibrary.NoOffException, ExceptionsLibrary.NoProductException, ExceptionsLibrary.NoAccountException {
        ...
        } catch (Server.ServerController.ExceptionsLibrary.NoOffException e) {
            e.printStackTrace();
        }
        for (String i : off.getOffProducts()){
            Product product = null;
            try {
                product = GetDataFromDatabaseServerSide.getProduct(Integer.parseInt(i));
            } catch (Server.ServerController.ExceptionsLibrary.NoProductException e) {
                e.printStackTrace();
            }
        ...
        for (File i : sellerFolder.listFiles(fileFilter)){
            ...
            Seller seller = null;
            try {
                seller = (Seller) GetDataFromDatabaseServerSide.getAccount(sellerUsername);
            } catch (Server.ServerController.ExceptionsLibrary.NoAccountException e) {
                e.printStackTrace();
            }
